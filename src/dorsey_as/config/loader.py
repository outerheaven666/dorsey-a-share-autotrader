...
def _parse_yaml(path: Path) -> dict[str, dict[str, Any]]:
    ...
    for raw_line in path.read_text(encoding="utf-8-sig").splitlines():
        ...
        data[current_section][key.strip()] = _parse_value(raw_value)
    return data
...
def validate_config(config: AppConfig) -> None:
    ...
    if config.report.output_format != "markdown":
        raise ConfigError("report.output_format currently only supports markdown.")
